class G {
  constructor(e, o, { tabInsertsSuggestions: i, firstOptionSelectionMode: a, scrollIntoViewOptions: r } = {}) {
    this.input = e, this.list = o, this.tabInsertsSuggestions = i ?? !0, this.firstOptionSelectionMode = a ?? "none", this.scrollIntoViewOptions = r ?? { block: "nearest", inline: "nearest" }, this.isComposing = !1, o.id || (o.id = `combobox-${Math.random().toString().slice(2, 6)}`), this.ctrlBindings = !!navigator.userAgent.match(/Macintosh/), this.keyboardEventHandler = (l) => W(l, this), this.compositionEventHandler = (l) => K(l, this), this.inputHandler = this.clearSelection.bind(this), e.setAttribute("role", "combobox"), e.setAttribute("aria-controls", o.id), e.setAttribute("aria-expanded", "false"), e.setAttribute("aria-autocomplete", "list"), e.setAttribute("aria-haspopup", "listbox");
  }
  destroy() {
...
var x = { MODULE_VERSION: "3.2.3", COMPRESSED_ORG: "GYJwhgtkA" };
...
function Be(t, e, o) {
  const i = t.action, a = j(new FormData(t), "pw-assistant"), r = Ge({ from: e, to: o }, a), l = new URLSearchParams(r);
  window.location.href = `${i}?${l.toString()}`;
}
function Fe(t, e) {
  const o = t.action, i = j(new FormData(t), "pw-departures"), a = We(i, e), r = new URLSearchParams(a);
