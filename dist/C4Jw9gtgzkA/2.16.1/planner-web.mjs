class H {
  constructor(e, i, { tabInsertsSuggestions: n, defaultFirstOption: a, scrollIntoViewOptions: s } = {}) {
    this.input = e, this.list = i, this.tabInsertsSuggestions = n ?? !0, this.defaultFirstOption = a ?? !1, this.scrollIntoViewOptions = s ?? { block: "nearest", inline: "nearest" }, this.isComposing = !1, i.id || (i.id = `combobox-${Math.random().toString().slice(2, 6)}`), this.ctrlBindings = !!navigator.userAgent.match(/Macintosh/), this.keyboardEventHandler = (u) => F(u, this), this.compositionEventHandler = (u) => N(u, this), this.inputHandler = this.clearSelection.bind(this), e.setAttribute("role", "combobox"), e.setAttribute("aria-controls", i.id), e.setAttribute("aria-expanded", "false"), e.setAttribute("aria-autocomplete", "list"), e.setAttribute("aria-haspopup", "listbox");
  }
  destroy() {
...
var B = { MODULE_VERSION: "2.16.1", COMPRESSED_ORG: "C4Jw9gtgzkA", ORG_ID: "troms" };
...
function D(t) {
  ...
  try {
    if (e && (e.valueAsDate = /* @__PURE__ */ new Date()), i) {
      const n = /* @__PURE__ */ new Date(), a = String(n.getHours()).padStart(2, "0"), s = String(n.getMinutes()).padStart(2, "0");
      i.value = `${a}:${s}`;
    }
  } catch {
  }
...
function Be(t, e, i) {
  const n = t.action, a = O(new FormData(t), "pw-assistant"), s = Re({ from: e, to: i }, a), u = new URLSearchParams(s);
  window.location.href = `${n}?${u.toString()}`;
}
function Oe(t, e) {
  const i = t.action, n = O(new FormData(t), "pw-departures"), a = Ge(n, e), s = new URLSearchParams(a);
...
function Pe({ URL_BASE: t }, e) {
  function i(r) {
    const c = a(r), h = v("span", [r.name]), d = v("span", [r.locality ?? ""], o.itemLocality), l = v("li", [c, h, d], o.listItem);
