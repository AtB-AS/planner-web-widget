class H {
  constructor(e, n, { tabInsertsSuggestions: i, defaultFirstOption: r, scrollIntoViewOptions: a } = {}) {
    this.input = e, this.list = n, this.tabInsertsSuggestions = i ?? !0, this.defaultFirstOption = r ?? !1, this.scrollIntoViewOptions = a ?? { block: "nearest", inline: "nearest" }, this.isComposing = !1, n.id || (n.id = `combobox-${Math.random().toString().slice(2, 6)}`), this.ctrlBindings = !!navigator.userAgent.match(/Macintosh/), this.keyboardEventHandler = (c) => W(c, this), this.compositionEventHandler = (c) => x(c, this), this.inputHandler = this.clearSelection.bind(this), e.setAttribute("role", "combobox"), e.setAttribute("aria-controls", n.id), e.setAttribute("aria-expanded", "false"), e.setAttribute("aria-autocomplete", "list"), e.setAttribute("aria-haspopup", "listbox");
  }
  destroy() {
...
var O = { MODULE_VERSION: "2.20.1", COMPRESSED_ORG: "GYJwhgtkA", ORG_ID: "fram" };
...
function D(t) {
  ...
  try {
    if (e && (e.valueAsDate = /* @__PURE__ */ new Date()), n) {
      const i = /* @__PURE__ */ new Date(), r = String(i.getHours()).padStart(2, "0"), a = String(i.getMinutes()).padStart(2, "0");
      n.value = `${r}:${a}`;
    }
  } catch {
  }
...
function B(t, e) {
    ...
    };
  {
    ...
    if (i && r) {
      const a = /* @__PURE__ */ new Date(`${i}T${r}`);
      return {
...
function Pe(t, e, n) {
  const i = t.action, r = B(new FormData(t), "pw-assistant"), a = Ve({ from: e, to: n }, r), c = new URLSearchParams(a);
  window.location.href = `${i}?${c.toString()}`;
}
function qe(t, e) {
  const n = t.action, i = B(new FormData(t), "pw-departures"), r = Qe(i, e), a = new URLSearchParams(r);
...
function He({ URL_BASE: t }, e, n) {
  function i(s) {
    const u = a(s), h = v("span", [s.name]), d = v("span", [s.locality ?? ""], o.itemLocality), l = v("li", [u, h, d], o.listItem);
