class j {
  constructor(t, o, { tabInsertsSuggestions: s, defaultFirstOption: i, scrollIntoViewOptions: l } = {}) {
    this.input = t, this.list = o, this.tabInsertsSuggestions = s ?? !0, this.defaultFirstOption = i ?? !1, this.scrollIntoViewOptions = l, this.isComposing = !1, o.id || (o.id = `combobox-${Math.random().toString().slice(2, 6)}`), this.ctrlBindings = !!navigator.userAgent.match(/Macintosh/), this.keyboardEventHandler = (a) => B(a, this), this.compositionEventHandler = (a) => R(a, this), this.inputHandler = this.clearSelection.bind(this), t.setAttribute("role", "combobox"), t.setAttribute("aria-controls", o.id), t.setAttribute("aria-expanded", "false"), t.setAttribute("aria-autocomplete", "list"), t.setAttribute("aria-haspopup", "listbox");
  }
  destroy() {
...
var C = { MODULE_VERSION: "1.0.0", COMPRESSED_ORG: "IYFwRkA" };
...
function Ce(e, t, o) {
  const s = e.action, i = D(new FormData(e)), l = Fe({ from: t, to: o }, i), a = new URLSearchParams(l);
  window.location.href = `${s}?${a.toString()}`;
}
function De(e, t) {
  const o = e.action, s = D(new FormData(e)), i = Ne(t, s), l = new URLSearchParams(i);
  ...
  class i extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.querySelector("button").addEventListener("click", async (_) => {
        ...
        S.clearMessageBox();
        try {
          const h = await Ge(), v = (f = r.parentElement) == null ? void 0 : f.querySelector("input");
