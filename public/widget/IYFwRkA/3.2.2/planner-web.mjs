class R {
  constructor(e, o, { tabInsertsSuggestions: i, firstOptionSelectionMode: a, scrollIntoViewOptions: r } = {}) {
    this.input = e, this.list = o, this.tabInsertsSuggestions = i ?? !0, this.firstOptionSelectionMode = a ?? "none", this.scrollIntoViewOptions = r ?? { block: "nearest", inline: "nearest" }, this.isComposing = !1, o.id || (o.id = `combobox-${Math.random().toString().slice(2, 6)}`), this.ctrlBindings = !!navigator.userAgent.match(/Macintosh/), this.keyboardEventHandler = (l) => W(l, this), this.compositionEventHandler = (l) => V(l, this), this.inputHandler = this.clearSelection.bind(this), e.setAttribute("role", "combobox"), e.setAttribute("aria-controls", o.id), e.setAttribute("aria-expanded", "false"), e.setAttribute("aria-autocomplete", "list"), e.setAttribute("aria-haspopup", "listbox");
  }
  destroy() {
  ...
  navigate(e = 1) {
    ...
    if (l)
      for (const f of i)
        f.removeAttribute("data-combobox-option-default"), l === f ? (this.input.setAttribute("aria-activedescendant", l.id), l.setAttribute("aria-selected", "true"), G(l), l.scrollIntoView(this.scrollIntoViewOptions)) : f.removeAttribute("aria-selected");
  }
  clearSelection() {
...
var F = { MODULE_VERSION: "3.2.2", COMPRESSED_ORG: "IYFwRkA" };
const Ie = 300, y = String.raw, T = F.MODULE_VERSION, L = F.COMPRESSED_ORG;
function Ce(t) {
...
function Me() {
  xe(), D("pw-assistant"), D("pw-departures");
  let t = {
    from: void 0,
    to: void 0
  };
...
function De(t, e, o) {
  const i = t.action, a = x(new FormData(t), "pw-assistant"), r = Re({ from: e, to: o }, a), l = new URLSearchParams(r);
  window.location.href = `${i}?${l.toString()}`;
}
function Be(t, e) {
  const o = t.action, i = x(new FormData(t), "pw-departures"), a = We(i, e), r = new URLSearchParams(a);
  ...
  class f extends HTMLElement {
    ...
    connectedCallback() {
      ...
      const N = async function() {
        $.clearMessageBox();
        try {
          const s = await Ke(t, e), m = u.parentElement?.querySelector("input");
      ...
      p.addEventListener("combobox-commit", function(s) {
        s.target.classList.contains("itemLocation") && N();
...
function qe(t) {
  return t.map(Oe).filter((e, o, i) => i.indexOf(e) === o);
}
function Oe(t) {
  switch (t) {
    case "onstreetBus":
